<?php
// includes/shortcode-basso-costo.php - VERSIONE FINALE E UNIFICATA

if ( ! defined( 'ABSPATH' ) ) exit;

add_shortcode( 'aos_basso_costo_vetrina', 'aos_display_basso_costo_vetrina_shortcode' );

/**
 * Funzione di callback per lo shortcode [aos_basso_costo_vetrina].
 *
 * Mostra operatrici per genere, con priorità per il genere specificato.
 * Gestisce correttamente le numerazioni per i generi "Basso Costo".
 *
 * @param array $atts Attributi dello shortcode. Richiede 'genere'.
 * @return string L'HTML della vetrina.
 */
function aos_display_basso_costo_vetrina_shortcode( $atts ) {
    ...
    // --- LOGICA DI ORDINAMENTO CON PRIORITÀ ---
    $altri_generi_slugs = [];
    ...
    if (is_wp_error($tutte_le_numerazioni) || empty($tutte_le_numerazioni)) { return '<p>Nessuna numerazione configurata.</p>'; }

    $mappa_genere_ddi = aos_get_ddi_map();

    $operatrici_per_vetrina = [];
    foreach ( $generi_da_mostrare as $genere_slug ) {
        $args = ['post_type' => 'operatrice', 'post_status' => 'publish', 'posts_per_page' => 1, 'orderby' => 'rand', 'tax_query' => [['taxonomy' => 'genere', 'field' => 'slug', 'terms' => $genere_slug]]];
        $query = new WP_Query( $args );
        if ( $query->have_posts() ) {
            $genere_term = get_term_by('slug', $genere_slug, 'genere');
            $operatrici_per_vetrina[] = ['post' => $query->posts[0], 'genere_nome' => $genere_term ? $genere_term->name : '', 'genere_slug' => $genere_term ? $genere_term->slug : ''];
        }
        wp_reset_postdata();
    }
    
    if (empty($operatrici_per_vetrina)) { return '<p>Nessuna operatrice disponibile per i generi specificati.</p>'; }
            ...
            <div>
                <div class="uk-card uk-padding-small uk-card-secondary uk-card-body uk-flex uk-flex-column uk-text-center cartomante" data-codice="<?php echo esc_attr($codice_da_tracciare); ?>">
                    ...
                    <div class="uk-margin-top">
                        <?php
                        // --- BLOCCO LOGICA FILTRAGGIO PULSANTI CORRETTO ---
                        ...
                        foreach ($tutte_le_numerazioni as $numerazione) {
                            ...
                            if ($is_basso_costo_card) {
                                ...
                                if (
                                    // Condizione 1: Mostra la numerazione specifica per il genere "Basso Costo" (es. 899)
                                    stripos($desc, $genere_nome) !== false ||
                                    // Condizione 2: Mostra "Carta di Credito" del genere base
                                    (stripos($desc, 'Carta di Credito') !== false && stripos($desc, $base_genere_nome) !== false) ||
                                    // Condizione 3: Mostra "Ricarica Online" del genere base
                                    (stripos($desc, 'Ricarica Online') !== false && stripos($desc, $base_genere_nome) !== false)
                                ) {
                                    $mostra_pulsante = true;
                                }
                            } else {
                                // Logica per i generi normali
                        ...
                        // --- FINE BLOCCO LOGICA FILTRAGGIO ---

                        // Stampa dei pulsanti (con la logica dei banner intatta come da te fornita)
                        if (!empty($numerazioni_filtrate)) {
                            usort($numerazioni_filtrate, function($a, $b) { 
                                $p_a = function_exists('aos_get_numerazione_priority') ? aos_get_numerazione_priority($a->description) : 0; 
                                $p_b = function_exists('aos_get_numerazione_priority') ? aos_get_numerazione_priority($b->description) : 0; 
                                return $p_a <=> $p_b; 
                            });
